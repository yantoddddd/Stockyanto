...
// ========== KONFIGURASI ==========
const WEBHOOK_SECRET = process.env.WEBHOOK_SECRET || 'whsec_AVu3fFLUBVMLjo6OdCWq7I3qdQ2CJ6e2';
...
async function setDB(products, orders, oldSha, retryCount = 0) {
    ...
    if (!res.ok) {
        ...
        if (errorData.message?.includes('SHA') || errorData.message?.includes('does not match')) {
            console.log(`⚠️ Webhook SHA conflict, retrying... (${retryCount + 1}/3)`);
...
async function sendTelegramNotification(order, bonusContent) {
    try {
        ...
        const message = `✅ <b>PEMBAYARAN BERHASIL!</b> (via Webhook)

📦 <b>Produk:</b> ${order.productName}
👤 <b>Pembeli:</b> ${order.customerName}
💰 <b>Total:</b> Rp ${(order.totalAmount || order.price || 0).toLocaleString()}
🆔 <b>Order ID:</b> ${order.orderCode}
